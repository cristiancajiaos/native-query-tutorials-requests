...
public class TutorialServiceImpl implements TutorialService {
  ...
  @Override
  public List<TutorialDTO> getTutorialsByLevelOrderAsc() {
    return tutorialRepository.getTutorialsByLevelOrderAsc().stream().map(this::convertToDTO).collect(
        Collectors.toList());
  }

  @Override
  public List<TutorialDTO> getTutorialsByLevelOrderDesc() {
    return tutorialRepository.getTutorialsByLevelOrderDesc().stream().map(this::convertToDTO).collect(
        Collectors.toList());
  }
  ...
  @Override
  public List<TutorialDTO> getTutorialsCreatedByDateBetween(Date startDate, Date endDate) {
    return tutorialRepository.getTutorialsCreatedByDateBetween(startDate, endDate).stream().map(this::convertToDTO).collect(
        Collectors.toList());
  }

  @Override
  public List<TutorialDTO> getTutorialsCreatedByDateAsc() {
    return tutorialRepository.getTutorialsCreatedByDateAsc().stream().map(this::convertToDTO).collect(
        Collectors.toList());
  }

  @Override
  public List<TutorialDTO> getTutorialsCreatedByDateDesc() {
    return tutorialRepository.getTutorialsCreatedByDateDesc().stream().map(this::convertToDTO).collect(
        Collectors.toList());
  }
